...
3. **Semantic cleanup (LLM)** — follow [references/cleanup-guide.md](references/cleanup-guide.md):
   ...
   - **Rebuild by topic blocks, not page-by-page**: group consecutive pages into logical
     blocks (a movement, a person, a concept); put a `##` block heading above each block,
     generated by the LLM when the source page has no clear heading.
   - Label every block with its page range right under the block heading
     (`<!-- 第 8-12 页 -->`); pages that merely continue the previous topic merge into
     that block instead of starting a new one.
   - Keep the `#` / `##` / `###` / `####` hierarchy uniform across the file.
...
4. **Verify** each file:
   ...
   - Every block has a `##` heading — no headingless content sitting right after a
     page marker;
   - Heading list has no fragment junk (`grep '^#'`);
   - Page ranges cover all content pages; cover/rest pages are explicitly marked
     (`第 N 页（休息页）`).
